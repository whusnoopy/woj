...
void AddControlClassProcessImp::process(int socket_fd, const string& ip, int length){
  LOG(INFO) << "Process add Control Class for:" << ip;
  char* buf;
  ...
  iter++;
  int ret = TeachInterface::getInstance().addControlClass(user_id, mclass);
  if (ret) {
  ...
  LOG(INFO) << "Process add Control Class completed for:" << ip;
}
